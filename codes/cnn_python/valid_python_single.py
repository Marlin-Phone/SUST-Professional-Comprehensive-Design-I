# valid_python_single_complex.py
# 用于验证复杂模型Numpy实现的脚本（单个推理）
...
if __name__ == '__main__':
    ...
    print('PyTorch Logits:', torch_output.numpy())
    print('Numpy Logits:', numpy_output)
    print('PyTorch Prediction:', label2emotion[torch_pred])
    print('Numpy Prediction:', label2emotion[numpy_pred])
    
    # 计算误差
    mean_error = np.mean(np.abs(torch_output.numpy() - numpy_output))
    print('Mean Error: {:.10f}'.format(mean_error))
