...
if __name__ == '__main__':
    ...
    import argparse
    opts = argparse.Namespace(num_classes=7)
    torch_model = LCNN(opts)
    
    # 加载训练好的权重
    import torch.serialization
    with torch.serialization.safe_globals([argparse.Namespace]):
        ckpt = torch.load('./results/best.ckpt', map_location='cpu', weights_only=False)
        torch_model.load_state_dict(ckpt['model_state_dict'])
    
    # PyTorch推理
    torch_model.eval()
    with torch.no_grad():
        torch_output = torch_model(img_tensor)
    
    # Numpy推理（模拟C语言实现）
    numpy_output = complex_model_inference(img_tensor, params)
    
    # 计算误差
    mean_error = np.mean(np.abs(torch_output.numpy() - numpy_output))
    print(f'复杂模型推理误差: {mean_error:.10f}')
    
    # 预测结果
    torch_pred = torch.argmax(torch_output, 1).item()
    numpy_pred = np.argmax(numpy_output, 1).item()
    
    label2emotion = ['surprise', 'fear', 'disgust', 'happiness', 'sadness', 'anger', 'neutral']
    
    print('PyTorch分类结果 (logits):')
    torch_logits = torch_output.numpy()[0]
    for i, emotion in enumerate(label2emotion):
        print(f"{emotion:10}: {torch_logits[i]:.4f}")
    
    print('Numpy分类结果 (logits):')
    numpy_logits = numpy_output[0]
    for i, emotion in enumerate(label2emotion):
        print(f"{emotion:10}: {numpy_logits[i]:.4f}")
    
    print(f'PyTorch预测: {label2emotion[torch_pred]}')
    print(f'Numpy预测:  {label2emotion[numpy_pred]}')
